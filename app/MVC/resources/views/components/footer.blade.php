    ...
    <div class="container py-4">
        <div class="row">
            <div class="col-md-4">
                ...
                <p>{{__('Dirección')}}: {{$propietario->direccio}}</p>
                <p>{{__('Teléfono')}}: +34 {{$propietario->telefon}}</p>
                <p>{{__('Correo electrónico')}}: {{$propietario->email}}</p>
            </div>
            <div class="col-md-4">
                <h3>{{__('Enlaces Rápidos')}}</h3>
                <ul class="list-unstyled">
                    <li><a href="{{ route('principal', ['id' => $PROPIETAT_ID]) }}" class="text-light">{{__('Inicio')}}</a></li>
                    <li><a href="{{route('reservas', ['id' => $PROPIETAT_ID])}}" class="text-light">{{__('Reservas')}}</a></li>
                    <li><a href="{{route('comentarios', ['id' => $PROPIETAT_ID])}}" class="text-light">{{__('Comentarios')}}</a></li>
                </ul>
            </div>
            <div class="col-md-4">
